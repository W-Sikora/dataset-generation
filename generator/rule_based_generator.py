...
from numpy import array, logical_xor
from numpy.random import choice, randn
from pandas import DataFrame
from util import generate_expression
...
def flip_(y, p):
    lst = [choice([1, 0], p = [p, 1 - p]) for i in range(len(y))]
    lst = array(lst)
    return array(logical_xor(y, lst), dtype = int)
...
x = DataFrame(gen_classification_symbolic(m='-9*x1**3',n_samples=100,flip_y=0.0))
x[2] = randn(x[0].shape[0])
print(x)
